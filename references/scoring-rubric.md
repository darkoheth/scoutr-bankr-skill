...
Mechanical cap: if deployer/launcher differs from fee recipient/project and no exact-token acknowledgement or fee claim is cited, alignment must be `please bro` or `pre-endorsement speculation`, Provenance must be <= 6, Confidence must be <= Medium, and endorsement evidence must be `none found for this CA`.
...
For `community-launched + endorsed`, the acknowledgement must be token-specific. A source that only confirms the project exists, confirms the founder, or acknowledges the project/product is not endorsement. The report should cite the exact token anchor: CA, Bankr/token page, ticker-as-token/cashtag, fee claim, or parent/quoted post that contains one.

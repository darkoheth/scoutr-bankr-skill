...
- Run the fee-recipient transfer check before finalizing `Endorsement evidence: none found`, `Fee-claim status: unknown`, `Fee-claim status: unclaimed`, `Alignment: please bro`, or `Alignment: pre-endorsement speculation` for a third-party/community Bankr launch. If matched transfers exist, they are direct claim evidence and exact-token acknowledgement: classify `Alignment: community-launched + endorsed`, cite the transfer, and mark `Fee-claim status: claimed`.
- Do not reject claim evidence just because an explorer transfer row lacks decoded method text, token-address fields, or `tx_hash`. Some internal transfer APIs return only token symbol, sender, recipient, amount, and timestamp. Match the exact Bankr fee-recipient wallet, launch/fee infrastructure sender, fee asset or launched token, and post-launch timing.

...
- Run a canonical-market check for non-Bankr/custom/DEX-native scans too: if exact Dexscreener/Gecko token-pairs expose a live pair, copy the selected pair's pool version, liquidity, FDV/MC, and volume. Do not leave liquidity unknown or guess the pool version from search prose.
...
- Do not add token-specific regression names, private contract addresses, or one-off fixture notes to public instructions. Keep regression coverage as behavior classes.
...
- Do not call chart/image generation for Scoutr. A final answer containing `![`, `pinata`, `ipfs://` chart media, or a hosted chart/image URL fails the output contract.
...
- If Bankr lookup returns no match, keep using structured exact market sources before synthesis. A non-Bankr result does not relax the requirement to copy canonical pair/liquidity/source links from Dexscreener/Gecko when they are available.
...
- Copy fee-claim status only from direct claim evidence. `claimed via Bankr metadata` is invalid unless the metadata actually exposes claim status; deployer/fee-recipient metadata alone is not a claim.
...
- If GitHub/code is private, only inferred from founder reputation, or not inspected, cap Code at 6. Do not call it `high-quality code`, `active repo`, or `strong code provenance` without a literal repo/org URL and age/history check. This is a Code/Product cap, not an automatic whole-report cap when the token thesis is mainly market/provenance/product traction.
...
- If liquidity, holder concentration, or provenance alignment is materially unchecked, do not return High confidence or `Trade Candidate`; cap the posture and say what evidence would unlock the higher score. If Code is materially unchecked, cap the Code score and only cap the whole verdict when code/product is central to the bullish thesis.
- If market structure is tiny, fading, or clearly downtrending, do not let good code/product alone produce `Trade Candidate`. Market weakness should appear in Token score, Red flags, and `My read`.
...
- Before writing `Endorsement evidence: none found` for a Bankr/Doppler community launch, apply the same fee-recipient transfer check. A matched fee-recipient transfer from launch/fee infrastructure is exact-token acknowledgement: write `Alignment: community-launched + endorsed`, `Endorsement evidence: fee claim transfer <asset/amount/source/time>`, and `Fee-claim status: claimed`.
- Do not require decoded method labels for fee-claim transfer evidence. Internal token-transfer rows can omit `claim`, `tx_hash`, or token-address fields; if the transfer matches the exact Bankr fee-recipient wallet, launched token or fee asset, launch/fee infrastructure sender, and timing, cite it as direct claim evidence.

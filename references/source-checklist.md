...
- Preserve structured results in a short source map before writing analysis. Required keys are token contract, chain, ticker, canonical pair, Dexscreener websites/socials, Bankr exact launch fields, Virtuals exact fields when present, fee-recipient/social/profile links, and GitHub candidates. The final report should be copied from this source map for source/provenance fields.

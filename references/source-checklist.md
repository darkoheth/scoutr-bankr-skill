...
- RepoScan is an optional outsourced deep-code analyzer for repo URLs only when configured through a no-per-scan-cost API key or free/internal integration. Use it for originality/similarity/trust-score analysis, then keep Scoutr responsible for attached-token discovery, token provenance, market/social checks, and final trade posture.
...
- Do not use RepoScan x402/pay-per-scan routes inside Bankr. A $0.05 per-scan endpoint is not suitable for default scouting volume.
- If RepoScan is unavailable, blocked, paid, unpaid, or times out, state `RepoScan: unavailable: <reason>; RepoScan-lite: used` and continue with the built-in RepoScan-lite checks.
- RepoScan-lite checks should capture the Zauth-like signals that are feasible without Zauth's private index: repo age/activity, stars/forks/license/topics, fork status, homepage, README specificity, package/config/docs/contracts/deploy/test/CI presence, commit/contributor summary when cheaply available, visible contract/token/social links, secret-risk patterns, generated/template/fork indicators, and exact-name/distinctive-phrase similarity searches.
- RepoScan-lite similarity is approximate. Use exact GitHub/search snippets, distinctive phrases, package names, fork/parent metadata, and copied-template markers when available, but do not claim "millions of repos" coverage or Zauth-equivalent similarity scoring.
- Include `RepoScan-lite score: <0-10>` when the fallback is used, and keep it scoped to GitHub/Product evidence. It must not replace attached-token discovery, launch provenance, market checks, social checks, or final verdict synthesis.

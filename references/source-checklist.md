...
- Liquidity, volume, FDV/market cap, pair age, LP lock/burn status, holder count, top-holder concentration. Use live structured token/pair/market-indexer data when available; do not reuse stale search summaries or chart-image estimates. For active Bankr/Base launches, do not use Blockscout as the primary holder-count source because it can lag badly; prefer Dexscreener, GeckoTerminal, Bankr/launch-platform data, BaseScan, or another live indexer for holder count, and use Blockscout mainly for source, transaction, wallet, and fee-claim evidence.
...
- Bankr-native market data: use `token_search`, `get_token_market_data`, or equivalent when available, but do not stop there if liquidity or primary pair is missing. Missing liquidity from Bankr-native output requires exact-CA Dexscreener token-pairs/search and/or GeckoTerminal fallback before finalizing.
- Bankr launch URLs are first-class inputs. If the user gives `https://bankr.bot/launches/<contract>`, extract `<contract>`, query Bankr exact lookup for that CA, and include the Bankr page in Sources/Source trace. Do not write Bankr no-match unless the exact lookup route failed with a stated blocker.
- Newer `ba3` CAs should get a normalized Bankr exact lookup retry before any no-match/custom conclusion.
...
- Market fallback rule: if any checked source returns FDV/volume/holders but not liquidity or primary pair, treat market data as incomplete and query another structured source. Prefer Dexscreener token-pairs/search, then GeckoTerminal exact token/pools. Only report `Liquidity: unknown` when all checked structured market routes are unavailable, empty, or explicitly missing liquidity.
- Market numbers must be copied from the selected live source with source/time context. If Dexscreener/token-pairs says liquidity is `$65k`, the report must not round it to `$100k` or infer a different value from FDV/volume. If holder sources disagree, report the freshest named holder source or mark `holders: source conflict`; do not treat Blockscout as authoritative for holder count on active Bankr/Base launches.
...
- Do not mark fees as `claimed` unless Bankr-native metadata, a claim transaction/event, an on-chain transfer from Bankr/Doppler/Airlock launch infrastructure to the fee-recipient wallet, or an explicit recipient statement proves fee claiming. A launch tweet, website link, or CA acknowledgement can prove endorsement but not fee claiming.
- Before writing `Fee-claim status: unclaimed` or `unknown` for a Bankr/Doppler community launch, inspect recent token transfers/transactions for the fee-recipient wallet and launch infrastructure when available. Fee claims can appear as WETH, USDC, EURC, the launched token, or another routed fee asset, not only as a Bankr API field.
- Treat a matched fee-recipient transfer as both fee-claim proof and exact-token acknowledgement for alignment. When the Bankr fee-recipient wallet receives launched-token/WETH/USDC/EURC fee transfers from launch/fee infrastructure for the scanned CA, classify a third-party launch as `community-launched + endorsed`, cite the transfer in `Endorsement evidence`, and use `Fee-claim status: claimed`.
- Explorer transfer rows may omit method names, decoded call labels, token addresses, or transaction hashes for internal transfers. Do not discard a valid claim solely because the row lacks `method: claim`; match on exact fee-recipient wallet, token/fee asset, launch/fee infrastructure sender, and timing relative to the Bankr/Doppler launch.
...
- Do not transfer reputation or metrics between similarly named repos/projects. If token/site/docs links `github.com/<project>/<repo>`, score that exact repo. Do not use stars/forks/commits from a famous upstream, dependency, compatible runtime, fork inspiration, or ecosystem neighbor unless the first-party source explicitly identifies it as the official repo for the token/project.
- Treat references such as "compatible with Nous/Hermes/OpenClaude", "built for MCP/x402", or "uses gitlawb" as compatibility/context only. They are not ownership, endorsement, employment, or token-official evidence without a direct first-party statement from that named project/team.

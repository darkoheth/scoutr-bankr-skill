...
- Liquidity, volume, FDV/market cap, pair age, LP lock/burn status, holder count, top-holder concentration. Use live structured token/pair/explorer data when available; do not reuse stale search summaries or chart-image estimates.
...
- Market numbers must be copied from the selected live source with source/time context. If Dexscreener/token-pairs says liquidity is `$65k`, the report must not round it to `$100k` or infer a different value from FDV/volume. If Blockscout says `74 holders`, the report must not report `210 holders` from stale/indexed/search context unless it names the fresher source.

...
- If on-chain/source evidence contains `AgentTokenV4`, `VirtualsBondingAdapter`, `virtualPersona`, `BONDING_V5`, or Virtuals-style adapter/factory labels, run Virtuals exact `tokenAddress` and `preToken` lookup before writing `custom`, `unknown`, `no live liquidity`, or `no token exists`. If Virtuals resolves the CA, its app/API record is canonical launch provenance and market context.
- Do not call a Virtuals token `self-launched`, `official`, `Trade Candidate`, or `Confidence: High` unless the report contains literal first-party URLs/handles and direct basis for that claim. For `UNDERGRAD` pre-tokens, cap at `Small Spec` / Medium by default when top-holder concentration is high, graduation is pending, or code/product/source fields are incomplete.
...
4. Reject blank or decorative source lines. `Website/docs:`, `X/social:`, `GitHub/code:`, `Launch tweet:`, or equivalent fields must contain a literal URL/handle, `not found after checking <specific sources>`, or `unavailable: <blocker>`. Empty markdown links, bare parentheses like `(Official)`, `(Founder)`, `(Verified product docs)`, or vague labels without URLs/handles are failed output.
   - Also reject `Website/docs: /`, `X/social:` with no handle/URL, and `GitHub/code: (Active org)` or any repo/org description without a literal GitHub URL.
...
- Fee claim evidence: inspect recent token transfers/transactions for the fee-recipient wallet and the launch hook/initializer/pool path. A transfer from Doppler/Airlock/Bankr launch infrastructure to the exact fee-recipient wallet in the launched token, WETH, USDC, EURC, or another routed fee asset is fee-claim evidence. If the claim evidence is found, use `Fee-claim status: claimed`, put the transfer source in `Endorsement evidence`, and treat a third-party launch as `community-launched + endorsed`.
- Transfer check order matters. For CA-only Bankr/Doppler scans where launcher/deployer and fee recipient differ, run the fee-recipient token-transfer check before writing `Endorsement evidence: none found`, `Fee-claim status: unknown`, `Fee-claim status: unclaimed`, or an unresolved `please bro`/`pre-endorsement speculation` verdict. If the check is unavailable, state the exact blocker instead of silently defaulting to unendorsed.
- Do not require the transfer row to have a decoded `claim` method name. Explorer token-transfer APIs may omit `tx_hash` or method labels for internal transfers; a timely incoming WETH/USDC/EURC/launched-token transfer from the launch/fee infrastructure to the Bankr fee-recipient wallet is enough direct claim evidence when the token, recipient wallet, and launch context match.
...
- If Bankr exact lookup returns `exactMatch` and launcher/deployer differs from fee recipient, immediately add fee-recipient wallet transfers to `source_map` or add `fee_claim_check: unavailable: <blocker>`. This field must be resolved before final alignment, endorsement evidence, fee-claim status, or verdict.
...
- Deployer affiliation must be proven, not guessed. If the Bankr deployer/launcher profile does not visibly identify itself as the project/team/founder, and checked first-party project sources do not name that deployer as part of the project, do not infer affiliation from follows, replies, ecosystem participation, mutual contacts, fee-recipient routing, or product quality.
- Prove every identity hop separately. Evidence that a founder/lead dev is tied to the project does not prove the Bankr deployer is that founder/lead dev. Evidence that the fee recipient is official does not prove the deployer is official. Do not merge `deployer`, `fee recipient`, `founder`, and `project` identities unless a checked source explicitly ties those exact handles/wallets together.
- Do not alias one handle/wallet to another person or handle in parentheses unless the alias is directly sourced in the same line. Forbidden without an alias source: `@deployer (Founder Name / @founder)`, `@deployer (project founder)`, `@deployer (official lead)`, or `@deployer (same as @feeRecipient)`. Use `@deployer (affiliation to <project> not found)` instead.
- Do not soften an unsourced alias into vague provenance wording. Phrases like `founder-linked wallet`, `known founder-linked deployer`, `official project lead`, `direct self-launch by a known builder`, `official token for the ecosystem`, or `alignment is perfect` require the same exact proof as `self-launched`: a source tying the deployer handle/wallet or exact CA to the founder/project.
- Do not write `verified affiliation via Bankr metadata` unless Bankr metadata explicitly states the affiliation relationship. Standard Bankr metadata that lists `deployer.xUsername` and `feeRecipient.xUsername` only proves those fields, not that the deployer is affiliated with the fee recipient/project.
...
- Direct fee-claim evidence is exact-token evidence. If the Bankr fee-recipient wallet receives launched-token/WETH/USDC/EURC fee transfers from launch/fee infrastructure for the scanned CA, `Endorsement evidence` must cite that transfer and the alignment for a third-party launch becomes `community-launched + endorsed`. Do not keep `Endorsement evidence: none found` after finding a valid fee-claim transfer.
...
- For Virtuals `UNDERGRAD` pre-tokens, use Virtuals page/API metrics first: holder count, market cap/FDV, liquidity, 24h volume, top-holder percentage, dev holding, factory, pre-token pair, verified socials, and project id. Dexscreener/Gecko pools can be secondary context but must not override the Virtuals factory page.
- For Virtuals `UNDERGRAD` pre-tokens, do not upgrade past `Small Spec` / Medium unless graduation/provenance/source fields are clean, top-holder concentration is not a material blocker, and product/code evidence includes literal inspected URLs.
...
- For CA-only Bankr scans, the default stop condition is: Bankr exact metadata captured, primary Dex/Gecko pair market data captured, current holder count/top-holder snapshot captured or blocked, first-party website/repo/docs links checked or blocked, and endorsement/fee-claim status checked or blocked. After that, write the report. Do not keep the job pending to chase broader sentiment or every possible holder transaction.
- A CA-only Bankr scan is not market-complete if Bankr-native market data omitted liquidity or primary pair and no Dexscreener/Gecko exact fallback was attempted. Treat this as a pre-send failure and perform the fallback or state the exact fallback blocker.
...
3. Select a provisional main pair by highest reliable liquidity and volume. Ignore obvious broken/stale pairs with impossible prices or tiny liquidity except as secondary markets.
...
5. Check Bankr provenance directly:
   ...
   - If the exact Bankr result has `poolId`, reconcile the market pair selection against that pool. When Dexscreener/Gecko returns the Bankr `poolId` as a live pair, that is the canonical pair for liquidity/FDV/volume unless it is clearly dead or broken; do not keep a lower-liquidity provisional side pool as primary.
...
- `Launcher/deployer: @deployer (Founder Name / @founder)`, `@deployer (project founder)`, or any `@deployer (<person> / @otherhandle)` alias without a cited source proving that exact alias. Identity aliases must be sourced, not inferred from project/founder/fee-recipient discovery.
- `Verified founder-linked wallet`, `official token for the ecosystem`, `self-launched by founder`, `official project lead`, `direct self-launch by a known builder`, or `alignment is perfect` when the deployer differs from the fee recipient and no source proves deployer control or exact-token acknowledgement.
...
- `Endorsement evidence: none found`, `Alignment: please bro`, or `Alignment: pre-endorsement speculation` after recent fee-recipient wallet transfers show launched-token/WETH/USDC/EURC transfers from Doppler/Airlock/Bankr launch infrastructure to the exact fee-recipient wallet for the scanned CA. Rewrite to `community-launched + endorsed` and cite the transfer.
...
2. Inspect token and market mechanics.
   ...
   - For Bankr/Doppler community launches, fee claims are part of provenance, not an optional afterthought. Before finalizing `please bro`, `pre-endorsement speculation`, `Fee-claim status: unknown`, or `Fee-claim status: unclaimed`, inspect recent fee-recipient token transfers when tools permit. Incoming WETH/USDC/EURC/launched-token transfers from launch/fee infrastructure to the exact Bankr fee-recipient wallet convert the launch to `community-launched + endorsed`.
...
3. Inspect social context.
   ...
   - For Bankr launches where deployer/launcher differs from fee recipient/project, search the official fee-recipient/project account and any claimed founder/lead-dev accounts for exact-token acknowledgement using the CA, Bankr launch/token page URL, ticker-as-token/cashtag, and words such as `fees`, `claim`, `claimed`, `support`, `supporting`, `thank you`, `thanks`, `building`, `Bankr`, `token`, and `launch`. Include official replies and quotes, not only top-level posts. If an official reply/quote acknowledges support or future use of fees in direct context of a post containing the exact CA/Bankr page/ticker-as-token, treat it as soft endorsement. If a founder/lead-dev post only announces the product/app/repo and does not mention the exact token context, score it under Product/Social only.

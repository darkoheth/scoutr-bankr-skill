...
3. Inspect social context.
   ...
   - For Bankr launches where deployer/launcher differs from fee recipient/project, search the official fee-recipient/project account for exact-token acknowledgement using the CA, Bankr launch/token page URL, ticker-as-token/cashtag, and words such as `fees`, `claim`, `claimed`, `support`, `supporting`, `thank you`, `thanks`, `building`, `Bankr`, `token`, and `launch`. Include official replies and quotes, not only top-level posts. If an official reply/quote acknowledges support or future use of fees in direct context of a post containing the exact CA/Bankr page/ticker-as-token, treat it as soft endorsement.

...
- For a direct repo URL, prefer RepoScan for deep repo/code analysis and similarity detection only when a no-per-scan-cost API key or free/internal integration is already configured.
...
- Do not use the public x402/pay-per-scan path inside Bankr. A $0.05 scan cost is too expensive for default Scoutr usage and can silently burn budget during repeated token scouting.

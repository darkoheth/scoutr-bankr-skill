...
- A founder/lead-dev product tweet is not token endorsement unless it contains or directly links the exact CA, Bankr/token page, ticker-as-token, fee claim, or explicit token acknowledgement. Do not write `launch tweet from <lead dev>` as endorsement unless the checked tweet is a token launch/acknowledgement post, not an app/repo/product announcement.
